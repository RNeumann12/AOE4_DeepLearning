...
def load_and_preprocess_data(config):
    ...
    # DIAGNOSTIC: Check enemy_civ structure
    print("\n" + "="*70)
    print("ENEMY_CIV DIAGNOSTIC")
    print("="*70)
    
    # Count how many unique values
    unique_enemy_civs = df['enemy_civ'].nunique()
    print(f"Unique enemy_civ values: {unique_enemy_civs}")
    ...
    # Show examples
    print("\nSample enemy_civ values:")
    print(df['enemy_civ'].value_counts().head(10))
    
    # Check within single games
    print("\nChecking variation within single games...")
    sample_game = df.groupby(['game_id', 'profile_id']).first().head(3)
    for idx, row in sample_game.iterrows():
        game_id, profile_id = idx
        game_data = df[(df['game_id'] == game_id) & (df['profile_id'] == profile_id)]
        unique_in_game = game_data['enemy_civ'].nunique()
        print(f"  Game {game_id}, Player {profile_id}: {unique_in_game} unique enemy_civ values in {len(game_data)} rows")
        if unique_in_game > 1:
            print(f"    Values: {game_data['enemy_civ'].unique()[:5]}")
...
class GameSequenceDataset(Dataset):
    ...
    def __init__(self, df, numerical_cols, categorical_cols):
        ...
        for (game_id, profile_id), group in grouped:
            ...
            # Extract features
            cat_features = group[categorical_cols].values  # Shape: (seq_len, n_cat_features)
            num_features = group[numerical_cols].values    # Shape: (seq_len, n_num_features)
...
def main():
    ...
    # IMPORTANT: These are runtime-dependent and should NOT be in wandb.config
    cat_vocab_sizes = [len(label_encoders[col].classes_) for col in config.categorical_cols]
    
    print(f"\nCategorical vocabulary sizes: {dict(zip(config.categorical_cols, cat_vocab_sizes))}")

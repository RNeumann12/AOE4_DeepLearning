...
class SequencePredictor(nn.Module):
    """
    Enhanced Causal Transformer for entity build order prediction.
    ...
    5. Multi-scale attention (local + global)
    ...
    def __init__(self,
        ...
        # 7. Improved transformer layers with alternating local/global attention
        ...
        # 8. Mixture of Experts (applied every other layer)
        if use_moe:
        ...
        # 10. Deep output head with skip connection and bottleneck
        ...
        # 11. Contrastive projection head (for auxiliary loss)
    ...
    def get_moe_aux_loss(self) -> torch.Tensor:
        """Get combined auxiliary loss from all MoE layers."""
        if not self.use_moe:
    ...
    def forward(self,
        ...
        condition = self.condition_proj(condition_concat).unsqueeze(1)  # (B, 1, d_model)
        ...
        # 5. Apply gated cross-attention for condition fusion
        entity_emb = self.condition_cross_attn(entity_emb, condition)
        ...
        if predict_next:
            ...
            head_out = self.entity_head(encoded)
            gated_hidden = encoded * (1 - self.output_gate) + head_out * self.output_gate
            ...
            if return_embeddings:
                ...
                mean_hidden = (encoded * mask_expanded).sum(dim=1) / mask_expanded.sum(dim=1).clamp(min=1)
...
class SequencePredictorTrainer:
    ...
    def train_step(self,
                ...
                with torch.cuda.amp.autocast():
                    if self.use_contrastive:
                        entity_logits, contrastive_emb = self.model(
                            entity_input, player_civ, enemy_civ, map_id, input_mask, 
                            predict_next=False, return_embeddings=True
                        )
                    else:
        ...
        if self.use_contrastive and 'contrastive_emb' in dir():
...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=160,
                       help='Batch size (balanced for 24GB VRAM)')
    parser.add_argument('--lr', type=float, default=2e-4,
                       help='Learning rate')
    parser.add_argument('--d_model', type=int, default=448,
                       help='Model dimension (larger for more capacity)')
    parser.add_argument('--nhead', type=int, default=8,
                       help='Number of attention heads')
    parser.add_argument('--num_layers', type=int, default=8,
                       help='Number of transformer layers')
    parser.add_argument('--dim_feedforward', type=int, default=1792,
                       help='FFN dimension (4x d_model)')
    parser.add_argument('--max_seq_len', type=int, default=128,
                       help='Maximum sequence length')
    parser.add_argument('--dropout', type=float, default=0.15,
                       help='Dropout rate (lower for larger model)')
    parser.add_argument('--grad_accum_steps', type=int, default=3,
                       help='Gradient accumulation steps (effective batch = 480)')
    ...
    parser.add_argument('--wandb_project', type=str, default='DeepLearning-SimpleBuildOrder-Enhanced',

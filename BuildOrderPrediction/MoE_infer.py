#!/usr/bin/env python3
"""Infer a build order using a trained MoE SequencePredictor model.

Example:
  python BuildOrderPrediction/MoE_infer.py --checkpoint best_model.pth --top_probs 5 --player_civ English --enemy_civ French --map "High View" --build_steps 30 --temperature 0.3 --top_p 0.9 --seed 42
  python BuildOrderPrediction/MoE_infer.py --checkpoint best_model.pth --top_probs 5 --player_civ sengoku_daimyo --enemy_civ French --map "High View" --build_steps 30 --temperature 0.3 --top_p 0.9 --seed 42
  """
import os
import sys
import argparse
...
from MoE_train import SequencePredictor
...
def load_model(checkpoint_path: str, device: torch.device):
    """Load MoE model and vocabularies from checkpoint."""
    ...
    architecture = checkpoint.get('architecture', 'EnhancedSequencePredictor')
...
def generate_build_order(
    ...
    Args:
        model: Trained SequencePredictor model
...
def pretty_print_build_order(
    ...
    print("\n" + "=" * 60)
    print("GENERATED BUILD ORDER (MoE Model)")
    print("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate build orders using trained MoE SequencePredictor'
    )
